...
void DebugPlane::update(std::shared_ptr<FrameState> frame_state) {
    if (!m_visible || !frame_state || !m_frameLabel || !m_batchLabel) {
        return;
    }
...
void DebugPlane::setVisible(bool visible) {
...
void DebugPlane::toggleVisible() {
    setVisible(!m_visible);
}

bool DebugPlane::isVisible() const {
    return m_visible;
}

void DebugPlane::applyVisibility() {
    if (m_frameLabel) {
        m_frameLabel->setVisible(m_visible);
    }
    if (m_batchLabel) {
        m_batchLabel->setVisible(m_visible);
    }
}
} // morrow
