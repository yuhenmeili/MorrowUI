...
void GLTFAnimationController::update(FrameStateSharedPtr frameState) {
    ...
    for (const auto& channel : anim.channels) {
        if (channel.samplerIndex < 0 ||
            channel.samplerIndex >= int(anim.samplers.size())) continue;
...
Quaternion GLTFAnimationController::lerpQuat(const std::vector<float>& output, int i, float f) {
    ...
    Quaternion a(output[i * 4 + 0], output[i * 4 + 1],
                  output[i * 4 + 2], output[i * 4 + 3]);
    Quaternion b(output[(i+1) * 4 + 0], output[(i+1) * 4 + 1],
                  output[(i+1) * 4 + 2], output[(i+1) * 4 + 3]);
