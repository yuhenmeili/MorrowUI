...
void EditorShell::refreshInspector(bool force) {
    ...
    for (const auto& property : properties) {
        signature << property.component << '\x1f' << property.name << '\x1f' << property.type << '\x1f' << (property.editable ? '1' : '0') << '\x1f'
                  << (property.mixed ? '1' : '0') << '\n';
    }
    ...
    const std::vector<std::string> propertyPriority = {
        "position", "rotation", "scale", "size", "mesh", "renderer_enabled", "material", "material_color", "blend_enabled", "double_sided", "visible",
    ...
    for (const auto& property : properties) {
        if (property.component != activeComponent) {
            ...
            m_inspectorPanel->addChild(header);
            y += 30.0f;
        }
        ...
        m_inspectorPanel->addChild(propertyLabel);

        if (property.type == "bool" && !property.mixed) {
            auto button = addButton(m_inspectorPanel, property.value == "true" ? L"[x]" : L"[ ]", controlX, y, controlWidth, 28.0f, [this, propertyName = property.name] {
            ...
            for (size_t index = 0; index < components.size(); ++index) {
                ...
                const std::array<Vector4, 4> componentColors = {
                    Vector4(0.90f, 0.40f, 0.43f, 1.0f), Vector4(0.55f, 0.80f, 0.35f, 1.0f), Vector4(0.38f, 0.64f, 0.94f, 1.0f),
