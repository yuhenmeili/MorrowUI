...
std::shared_ptr<morrow::MRButton> makeBox(const morrow::Vector4& color, bool interactive) {
...
std::shared_ptr<RenameNodeDialog> RenameNodeDialog::create() {
    auto value = std::shared_ptr<RenameNodeDialog>(new RenameNodeDialog());
...
void RenameNodeDialog::initializeControls() {
    m_backdrop = makeBox(Vector4(0.015f, 0.02f, 0.03f, 0.58f), true);
    addChild(m_backdrop);
    ...
    m_nameEdit->setBackgroundColor(Vector4(0.07f, 0.08f, 0.10f, 1.0f));
    m_nameEdit->setFocusedBackgroundColor(Vector4(0.10f, 0.13f, 0.18f, 1.0f));
    m_nameEdit->setTextColor(Vector4(0.95f, 0.96f, 0.98f, 1.0f));
    m_submitConnection = m_nameEdit->events().onSubmitted.connect([this](MRTextEdit&, const std::wstring&) { confirm(); });
    m_panel->addChild(m_nameEdit);

    m_confirmButton = MRButton::create();
    m_confirmButton->setText(L"Rename", "default");
    m_confirmButton->setBackgroundColor(Vector4(0.18f, 0.34f, 0.59f, 1.0f));
    m_confirmButton->setHoverColor(Vector4(0.27f, 0.49f, 0.78f, 1.0f));
    m_confirmConnection = m_confirmButton->events().onClicked.connect([this](BaseButton&) { confirm(); });
    ...
    m_cancelConnection = m_cancelButton->events().onClicked.connect([this](BaseButton&) { hideDialog(); });
    m_panel->addChild(m_cancelButton);
}

void RenameNodeDialog::show(const std::string& nodeId, const std::string& currentName) {
    m_nodeId = nodeId;
...
void RenameNodeDialog::layoutControls() {
    ...
    m_confirmButton->getTransform()->setPosition(70.0f, 124.0f, 0.0f);
    m_confirmButton->getTransform()->setSize(140.0f, 38.0f);
    m_cancelButton->getTransform()->setPosition(270.0f, 124.0f, 0.0f);
