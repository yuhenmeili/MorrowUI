...
- 验收：新增 `BackblurDemo`（全屏卡片墙 + 若干模糊面板叠加，含普通 UI
  ...
  —— DebugDemo 回归通过（14/5/5 基线一致）。排查期间的 ImageDemo
  段错误已定位并修复：CommandBuffer payload 仅 8 字节对齐，Release
  下 payload 构造被向量化为 movaps（要求 16 对齐）而确定性崩溃；
  已改为 16 字节对齐布局（编码 / 解码 / 溢出槽同步更新）。
